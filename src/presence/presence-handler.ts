...
type QueryCallback = (
  error: EVENT,
  data?: QueryResult | IndividualQueryResult,
) => void;

function validateQueryArguments(
  // deno-lint-ignore no-explicit-any
  rest: any[],
): { users: string[] | null; callback: null | QueryCallback } {
...
export class PresenceHandler {
  ...
  subscribe(
    userOrCallback: string | SubscribeCallback,
    callback?: SubscribeCallback,
  ): void {
    if (
      typeof userOrCallback === "string" && userOrCallback.length > 0 &&
  ...
  unsubscribe(
    ...
    if (
      userOrCallback && typeof userOrCallback === "string" &&
      userOrCallback.length > 0
    ) {
      const user = userOrCallback;
      if (callback) {
  ...
  getAll(
    callback: (error: { reason: EVENT }, result?: QueryResult) => void,
  ): void;
  getAll(
    users: string[],
    callback: (
      error: { reason: EVENT },
      result?: IndividualQueryResult,
    ) => void,
  ): void;
  getAll(
    // deno-lint-ignore no-explicit-any
    ...rest: any[]
  ): Promise<QueryResult | IndividualQueryResult> | void {
    ...
    return new Promise<QueryResult | IndividualQueryResult>(
      (resolve, reject) => {
        emitter.once(
          emitterAction,
          (
            error: { reason: string },
            results: QueryResult | IndividualQueryResult,
