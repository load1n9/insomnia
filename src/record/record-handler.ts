...
import { AnonymousRecord } from "./anonymous-record.ts";
import { List } from "./list.ts";
import { ListenCallback, Listener } from "../util/listener.ts";
import { SingleNotifier } from "./single-notifier.ts";
import { WriteAcknowledgementService } from "./write-ack-service.ts";
import { DirtyService } from "./dirty-service.ts";
import { MergeStrategyService } from "./merge-strategy-service.ts";
...
export class RecordHandler {
  ...
  constructor(
    ...
    private listener: Listener = new Listener(TOPIC.RECORD, services),
  ) {
    this.recordServices = recordServices || {
      bulkSubscriptionService: {
        [RECORD_ACTION.SUBSCRIBECREATEANDREAD]: this.getBulkSubscriptionService(
          RECORD_ACTION.SUBSCRIBECREATEANDREAD,
        ),
        [RECORD_ACTION.SUBSCRIBEANDREAD]: this.getBulkSubscriptionService(
          RECORD_ACTION.SUBSCRIBEANDREAD,
        ),
        [RECORD_ACTION.SUBSCRIBEANDHEAD]: this.getBulkSubscriptionService(
          RECORD_ACTION.SUBSCRIBEANDHEAD,
        ),
      },
      writeAckService: new WriteAcknowledgementService(services),
      readRegistry: new SingleNotifier(
        services,
        RECORD_ACTION.READ,
        options.recordReadTimeout,
      ),
      headRegistry: new SingleNotifier(
        services,
        RECORD_ACTION.HEAD,
        options.recordReadTimeout,
      ),
      dirtyService: new DirtyService(
        services.storage,
        options.dirtyStorageName,
      ),
  ...
  setMergeStrategyRegExp(regexp: RegExp, mergeStrategy: MergeStrategy): void {
    if (typeof mergeStrategy === "function") {
      this.recordServices.mergeStrategy.setMergeStrategyByPattern(
        regexp,
        mergeStrategy,
      );
    } else {
  ...
  snapshot(
    name: string,
    callback: (error: string | null, data: RecordData) => void,
  ): void;
  snapshot(
    name: string,
    callback?: (error: string | null, data: RecordData) => void,
  ): void | Promise<RecordData> {
    ...
    if (callback) {
      ...
      return new Promise((resolve, reject) => {
        this.recordServices.readRegistry.request(
          name,
  ...
  has(
    name: string,
    callback: (error: string | null, has: boolean | null) => void,
  ): void;
  has(
    name: string,
    callback?: (error: string | null, has: boolean | null) => void,
    ...
    if (!callback) {
      return new Promise((resolve, reject) => {
        cb = (error: string | null, version: number) =>
          error ? reject(error) : resolve(version !== -1);
        this.head(name, cb);
      });
    }
    cb = (error: string | null, version: number) =>
      error ? callback(error, null) : callback(null, version !== -1);
    this.head(name, cb);
  }
  ...
  head(
    name: string,
    callback: (error: string | null, version: number) => void,
  ): void;
  head(
    name: string,
    callback?: (error: string | null, version: number) => void,
    ...
    if (callback) {
      ...
      return new Promise((resolve, reject) => {
        this.recordServices.headRegistry.request(
          name,
          (error, data) => error ? reject(error) : resolve(data),
        );
      });
  ...
  setData(
    recordName: string,
    path: string,
    data: RecordPathData | undefined,
    callback: WriteAckCallback,
  ): void;
  setData(
    recordName: string,
    pathOrData: string | RecordData,
    dataOrCallback: RecordPathData | WriteAckCallback | undefined,
    callback?: WriteAckCallback,
  ): void;
  setData(recordName: string): void {
    const args = utils.normalizeSetArguments(arguments, 1);
    this.sendSetData(recordName, -1, args);
  }
  ...
  delete(
    _recordName: string,
    _callback?: (error: string | null) => void,
  ): void | Promise<void> {
  ...
  notify(
    ...
    if (!this.services.connection.isConnected) {
      ...
      return new Promise((_resolve, reject) => reject(EVENT.CLIENT_OFFLINE));
    }
  ...
  private sendSetData(
    recordName: string,
    version: number,
    args: utils.RecordSetArguments,
  ): void {
    const { path, data, callback } = args;
    if (
      !recordName || typeof recordName !== "string" || recordName.length === 0
    ) {
      throw new Error(
  ...
  clearOfflineStorage(
    ...
    if (callback) {
      ...
      return new Promise((resolve, reject) => {
        this.services.storage.reset((error) =>
